...
IMAGES_DIR         = CELLPHONE_DB_DIR / Path('images')
# ------------------------------------------
# Setup logging
...
if __name__ == "__main__":  
    ...
    SCANVI_LATENT_KEY = "Integration_scANVI"
    LEIDEN_RESOLUTION=.6
    input_file = ROOT_PATH / Path(DATA_DIR) / Path('0a0a_'+RESULTS_DIR) / Path(f'{number_of_samples_to_keep}_samples_integration_{SCANVI_LATENT_KEY}_images_Leiden_resolution_{LEIDEN_RESOLUTION}')        
    ...
    adata.X = adata.layers['counts'].astype('float32')
    ...
    # -------------------------------        
    selected_cell_types = ['PT','PT_MT','PT_VCAM1','PT_PROM1','ENDO','PODO','LOH','PEC','FIB_VSMC_MC','LEUK','DCT1','DCT2','PC','TAL1','TAL2','MES']
    # adata = adata[adata.obs['celltype'].isin(selected_cell_types)].copy()
    adata.obs['cell_labels'] = adata.obs['celltype']
    ...
    ov.pl.embedding(adata,
            #    basis='Integration_scANVI',
               basis='X_umap',
               color='cell_labels',
               frameon='small',
               palette=ov.pl.red_color+ov.pl.blue_color+ov.pl.green_color+ov.pl.orange_color+ov.pl.purple_color)
    plt.savefig(IMAGES_DIR/"embedding_plot_x_umap.png", dpi=300, bbox_inches='tight')
    # -------------------------------
    ...
    fig.savefig(IMAGES_DIR/"cpdb_heatmap.png", dpi=300, bbox_inches='tight')

    fig, ax = plt.subplots(figsize=(2,4)) 
    ov.pl.cpdb_heatmap(adata,interaction['interaction_edges'],celltype_key='cell_labels',
                    source_cells=['PT','PT_MT','PT_VCAM1','PT_PROM1'],
            ax=ax,legend_kws={'fontsize':12,'bbox_to_anchor':(5, -0.9),'loc':'center left',})
    fig.savefig(IMAGES_DIR/"selected_cpdb_heatmap.png", dpi=300, bbox_inches='tight')

    # fig=ov.pl.cpdb_chord(adata,
    ...
    ov.pl.cpdb_network(adata,interaction['interaction_edges'],celltype_key='cell_labels',
                ...
                nodesize_scale=5,
                    ax=ax)  
    fig.savefig(IMAGES_DIR/"cpdb_network.png", dpi=300, bbox_inches='tight')
    ...
    ov.pl.cpdb_network(adata,interaction['interaction_edges'],celltype_key='cell_labels',
                ...
                source_cells=['PT','PT_MT','PT_VCAM1','PT_PROM1'],
                ax=ax)
    fig.savefig(IMAGES_DIR/"cpdb_network2.png", dpi=300, bbox_inches='tight')
    ...
    ov.single.cpdb_plot_network(adata=adata,
                    ...
                    legend_bbox=(0.8,0.2),
                    legend_fontsize=10)    
    fig.savefig(IMAGES_DIR/"cpdb_network3.png", dpi=300, bbox_inches='tight')
    ...
    ov.pl.cpdb_interacting_network(adata=adata,
                            celltype_key='celltype',
                                ...
                                figsize=(6,6),
                                fontsize=10)
    fig.savefig(IMAGES_DIR/"cpdb_network4.png", dpi=300, bbox_inches='tight')

    sub_i=interaction['interaction_edges']
    sub_i=sub_i.loc[sub_i['SOURCE'].isin(selected_cell_types)]
    sub_i=sub_i.loc[sub_i['TARGET'].isin(selected_cell_types)]
    sub_adata=adata[adata.obs['cell_labels'].isin(selected_cell_types)]
    ...
    fig.savefig(IMAGES_DIR/"cpdb_network5.png", dpi=300, bbox_inches='tight')


    sub_means=ov.single.cpdb_exact_target(cpdb_results['means'],['PT'])
    sub_means=ov.single.cpdb_exact_source(sub_means,['PT_MT','PT_VCAM1','PT_PROM1'])
    sub_means.head() 
    ...
    ov.pl.cpdb_interacting_heatmap(adata=adata,
                            celltype_key='cell_labels',
                                ...
                                source_cells=['PT_MT','PT_VCAM1','PT_PROM1'],
                                target_cells=['PT'],
                                plot_secret=True,
                                min_means=3,
                                nodecolor_dict=None,
                                ...
                                figsize=(2,6),
                                fontsize=10,)
    fig.savefig(IMAGES_DIR/"cpdb_network6.png", dpi=300, bbox_inches='tight')
    ...
    pathway_dict=ov.utils.geneset_prepare(str(ENRICHMENT_ANALYSIS_GENE_SETS/"KEGG_2021_Human.txt"),organism='Human')  
    ...
    ov.plot_set()
    ov.bulk.geneset_plot(enr,figsize=(2,4),fig_title='KEGG(PT-PT_MT/VCAM1/PROM1)',
                        cax_loc=[2, 0.45, 0.5, 0.02],num=8,
                        bbox_to_anchor_used=(-0.25, -13),custom_ticks=[10,100],
                        cmap='Greens')
    plt.savefig(IMAGES_DIR/"genes.png", dpi=300, bbox_inches='tight')
