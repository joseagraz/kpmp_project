...
if __name__ == "__main__":  
    ...
    # ov.utils.retrieve_layers(adata.copy(),layers='counts')
    # -------------------------------
    random_indices = np.random.choice(adata.n_obs, 5000, replace=False)
    ...
    print('Inference of MetaCell from Single-Cell RNA-seq')
    meta_obj=ov.single.MetaCell(adata,use_rep='scaled|original|X_pca',
                                n_metacells=None,
                                use_gpu='cuda:0')
    print('Initialize the archetypes')
    meta_obj.initialize_archetypes()
    print('Train the model')
    meta_obj.train(min_iter=10, max_iter=50)
    meta_obj.save(OUTPUT_DIR/'model.pkl')
    meta_obj.load(OUTPUT_DIR/'model.pkl')
