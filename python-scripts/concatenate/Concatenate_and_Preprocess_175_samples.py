...
RIBO_DEFINITION_FILE_PATH = "/mnt/12TB_Disk/KPMP_Data/Privately_Available_Data/Supporting_Files/KEGG_RIBOSOME.v2023.2.Hs.txt"
# 12TB disk path
...
def ribo_genes_removal(adata_original):
    ...
    ribo_genes        = pd.read_table(RIBO_DEFINITION_FILE_PATH, skiprows=2, header = None)
    ...
    return adata_original,num_filtered     
# ------------------------------------------
...
def process_sample(data_location,sample_name):
    ...
    csv_file_path = root_path / Path(support_files_dir) / 'list_of_removals.csv'
    ...
    # Store the number of cells before processing
    num_cells_before = adata.n_obs
    num_genes_before = adata.n_vars
    ...
    # Store the number of cells after processing
    num_cells_after = adata.n_obs    
    num_genes_after = adata.n_vars
...
if __name__ == "__main__":    
    ...
    completed_samples = 0
    removed_list=[]
    # read_and_process_data(sample_names[2], root_path, data_dir, source_data_path)
