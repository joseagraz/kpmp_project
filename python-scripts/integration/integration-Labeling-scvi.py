...
if __name__ == "__main__":
    ...
    # ------------------------------------------
    input_file = OUTPUT_DIR / 'single_cell_harmony_results_file.h5ad'
    adata=sc.read_h5ad(input_file)
    adata.var_names_make_unique()
    adata.obs_names_make_unique()    
    # ------------------------------------------
    ref_adata = sc.read_h5ad(ATLAS_FILE_NAME)
    ref_adata.var_names_make_unique()
    ref_adata.obs_names_make_unique()    
    ref_adata.obs  = ref_adata.obs[[COLUMN_OF_INTEREST]]  
    ref_adata.var.rename(columns={'features-0': 'features'}, inplace=True)
    ref_adata.var  = ref_adata.var[['features']]        
    # print(ref_adata.obs.celltype.value_counts())   
    # ------------------------------------------
    cell_mapper   = produce_cell_map(adata,ref_adata)
    adata.obs[COLUMN_OF_INTEREST] = adata.obs.index.map(cell_mapper)
    # ------------------------------------------
    output_file = OUTPUT_DIR / 'single_harmony_cell_labeled_results_file.h5ad'
    adata.write_h5ad(output_file)
    adata=sc.read_h5ad(output_file) # Need to test saved file, sometimes the file corrupts
    # ------------------------------------------                 
    print('Done')
